    ...
    <div class="playVideo">
      <div class="video-container">
        <div class="video-display">  
          <h2 class="display-title">当前播放视频</h2>  
          <div class="video-name-container">  
            <div class="video-name-scroll">  
              {{ video_name }}  
            </div>  
          </div>  
        </div>
        ...
        <div class="video-selector"> 
          ...
          >
            ...
            <div v-else>
              ...
              >
                <el-table-column prop="name" label="视频名称" min-width="150"></el-table-column>
                <el-table-column label="操作" width="150" align="center">
                  <template slot-scope="scope">
                    <div class = "select-video">
                      <el-button size="mini" type="primary" @click.stop="onDeleteClick(scope.row.name)">删除</el-button>
                      <el-button size="mini" type="primary" @click.stop="onSelectClick(scope.row.name)">选择</el-button>
                    </div>
                  </template>
                </el-table-column>
              </el-table>
        ...
        <div class="container1">  
          <button type="primary" class="upload-btn" @click="openUploadDialog">
              上传视频
          </button>
        </div> 
        <!-- 选择视频按钮 -->
        <div class="container1">  
          <button type="primary" class="upload-btn" @click="fetchAndShowDialog" :loading="isFetching">选择视频</button>  
        </div> 
        <div class="container1">
          <!-- 触发按钮 -->
          <button  type="primary" @click="showGuide" class="upload-btn">
            快捷按键
          </button>
        </div>
        <div class="container1">
          <!-- 触发按钮 -->
          <button  type="primary" @click="backTostart" class="upload-btn">
            跳到开始
          </button>
        </div>
        <div class="container1">
          <!-- 触发按钮 -->
          <button  type="primary" @click="backToend" class="upload-btn">
            跳到末尾
          </button>
        </div>
      </div>
      <div class="video-play">
        <video controls id="myVideo" ref="videoPlayer" :controls="false" @ended="handleVideoEnded">
      ...
      <div class="menu">
        <div class="controlMenu">
          ...
          <div class="contorlBtn">
            <el-button type="primary" size="mini" @click="serveSubmit()">提交视频</el-button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    showGuide() {
            this.$alert(
                '<div style="text-align:center;"><img src="/quickKey.jpg" style="max-width:100%;"></div>',
                '快捷键',
    ...
    async send(){
      ...
      for(var i = 0; i < this.cutCoverList.length; i++)
      {
          requestData.segments.push({output_name:'',start_time:'',end_time:''})
          requestData.segments[i].output_name=this.cutCoverList[i].text
          requestData.segments[i].start_time=this.cutCoverList[i].startTime
    ...
    backTostart() {
      ...
      this.Event.$emit("currentTime", this.timeCurrentLeft);
      console.log("currentTime",this.timeCurrentLeft)
    },
  ...
  watch: {
    ov_list:{
      handler() {
      this.rawVideoList = this.mode === true ? this.ov_list : this.cv_list;
    },
    deep: true // 深度监听
    },
    cv_list:{
...
.video-play {
  ...
  box-shadow: 0 8px 20px rgba(0, 0, 0, 0.2); /* 立体阴影 */
  background: #000; /* 默认背景色（视频加载前） */
  aspect-ratio: 16/9; /* 保持16:9比例（可选） */
  // background-color: #eee9e9;
  // border: 2px solid #409eff;
}
...
.custom-alert-dialog {
  width: 1000px !important; /* 设置宽度 */
  max-width: 90% !important; /* 最大宽度 */
}
</style>
